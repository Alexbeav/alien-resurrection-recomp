...
#
# Writes: dist/alien-resurrection-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix alien-resurrection-recomp \
  --exe-name Alien_Resurrection_Recompiled \
  --display-name "Alien Resurrection Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Alien Resurrection disc" \
